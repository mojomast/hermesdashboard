...
- opportunistic title/summary backfill during session list and session detail reads must never make those read endpoints fail; if a local SQLite write collides on a unique title constraint or the database is temporarily locked, the dashboard should fall back to read-only behavior and keep rendering the existing session rows
