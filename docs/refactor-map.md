...
- Added optional Dashboard Chat / IRC availability as a hidden-by-default tab with privacy-preserving settings, `GET /api/dashboard-chat/status`, gated `/api/dashboard-chat/ws`, pure sanitizer/parser services, no-network disabled tests, and source-contract coverage via `dashboard_source()`.
