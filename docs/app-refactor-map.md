...
- **dashboard-chat / IRC message:** Optional external IRC bridge event scoped to `#hermesdashboard`; distinct from main Hermes chat runs and durable Message Board posts. Opening the tab is read-only; websocket network access requires explicit `dashboard_chat.enabled` plus user Connect.
