...
2. **Patch all route surfaces together**
   ...
   - For the active single-file dashboard at `~/.hermes/dashboard`, inspect `templates/index.html` specifically: a tab may have `data-panel="name"` and an existing `id="name-panel"`, but still fail because `handleHashChange()` has a separate `validPanels` allowlist. Add the panel to that allowlist and to the breadcrumb `names` map, then add a regression test like `tests/test_<panel>_panel_navigation.py` asserting the nav button, panel id, router allowlist entry, lazy-loader case, and breadcrumb label.
...
4. **Verify in two layers**
   - Rebuild the dashboard frontend bundle, e.g. `npm run build` in the dashboard project.
   ...
   - If a dashboard server was already running, restart it for backend Python changes and rebuilt static assets. For Hermes dashboard on the default local port, identify the process with `ss -ltnp` / `ps`, stop only the dashboard PID, then relaunch from the repo, e.g. `/home/mojo/.local/bin/hermes dashboard --no-open --port 9119`.
...
- A successful frontend build does not prove backend route/title helpers were reloaded; smoke-test the API and mention restart if needed.
